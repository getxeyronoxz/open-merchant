...
export function CompetitorsScreen({ currency, competitors, evidence, onSave, statistics }: { currency: string; competitors: Competitor[]; evidence: EvidenceSource[]; onSave: (competitors: Competitor[]) => Promise<void>; statistics?: CompetitorStatistics; }) {
  ...
  return <section className="rounded-xl border border-stone-800 bg-stone-900 p-6">
    ...
    {statistics?.validPriceCount ? <div className="mt-6 grid gap-3 sm:grid-cols-4">{([ ["Minimum", statistics.minimum], ["Median", statistics.median], ["Average", statistics.average], ["Maximum", statistics.maximum] ] as const).map(([label, value]) => <div className="rounded-lg border border-stone-800 bg-stone-950/60 p-3" key={label}><p className="text-xs font-medium uppercase tracking-wide text-stone-500">{label}</p><p className="mt-1 text-lg font-semibold">{value ? formatCurrency(value, currency) : "—"}</p></div>)}</div> : null}
