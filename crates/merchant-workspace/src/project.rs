...
use merchant_core::{
    validation::{
        validate_assumptions, validate_competitors, validate_currency, validate_evidence_sources,
...
impl Workspace {
    ...
    pub fn load_snapshot(&self) -> Result<ProjectSnapshot, WorkspaceError> {
        ...
        let manifest: ProjectManifest =
            serde_json::from_str(&raw).map_err(|source| WorkspaceError::Json {
                path: path.clone(),
                source,
            })?;
    ...
    pub fn save_competitors(&self, competitors: &[Competitor]) -> Result<(), WorkspaceError> {
        ...
        for competitor in competitors {
            writer
                .serialize(CompetitorCsv::from(competitor))
                .map_err(|source| WorkspaceError::Csv {
                    path: path.clone(),
                    source,
                })?;
        }
        writer.flush().map_err(|source| WorkspaceError::Io {
            path: path.clone(),
            source,
        })?;
    ...
    pub fn load_assumptions(&self) -> Result<CostAssumptions, WorkspaceError> {
        ...
        let assumptions = serde_json::from_str::<CostAssumptions>(&contents).map_err(|source| {
            WorkspaceError::Json {
                path: path.clone(),
                source,
            }
    ...
    pub fn save_assumptions(&self, assumptions: &CostAssumptions) -> Result<(), WorkspaceError> {
        ...
        validate_assumptions(assumptions, &self.load_snapshot()?.manifest.currency)
            .map_err(|error| WorkspaceError::Validation(error.to_string()))?;
        let json =
            serde_json::to_vec_pretty(assumptions).map_err(|source| WorkspaceError::Json {
                path: path.clone(),
                source,
            })?;
        atomic::write(&path, &with_newline(json))
        ...
        for scenario in scenarios {
            writer
                .write_record([
                    ...
                    scenario.selling_price.file_string(),
                    scenario.acquisition_cost.file_string(),
                    scenario.shipping_cost.file_string(),
                    scenario.marketplace_fee_rate.file_string(),
                    scenario.marketplace_fee.file_string(),
                    scenario.payment_fee_rate.file_string(),
                    scenario.payment_fee.file_string(),
                    scenario.other_costs.file_string(),
                    scenario.total_cost.file_string(),
                    scenario.gross_profit.file_string(),
                    scenario.gross_margin_percent.file_string(),
                ])
                .map_err(|source| WorkspaceError::Csv {
                    path: path.clone(),
                    source,
...
fn scenario_name(scenario: &EconomicsScenario) -> String {
    match scenario.scenario {
        merchant_core::ScenarioName::Low => "low",
        merchant_core::ScenarioName::Base => "base",
...
fn competitor_from_csv(row: CompetitorCsv) -> Result<Competitor, WorkspaceError> {
    let price = if row.price.trim().is_empty() {
        None
    } else {
        Some(
            DecimalString::parse(&row.price)
                .map_err(|error| WorkspaceError::Validation(error.to_string()))?,
        )
    };
    let observed_at = DateTime::parse_from_rfc3339(&row.observed_at)
        .map_err(|error| {
            WorkspaceError::Validation(format!("invalid competitor timestamp: {error}"))
        })?
        .with_timezone(&Utc);
    Ok(Competitor {
...
fn initialize_files(root: &Path, manifest: &ProjectManifest) -> Result<(), WorkspaceError> {
    for relative in paths::ALL_ARTIFACTS {
        ...
        let parent = path
            .parent()
            .expect("workspace artifact paths always have parents");
    ...
    let manifest_json =
        serde_json::to_vec_pretty(manifest).map_err(|source| WorkspaceError::Json {
            path: paths::at(root, paths::MANIFEST),
            source,
        })?;
    atomic::write(
        &paths::at(root, paths::MANIFEST),
        &with_newline(manifest_json),
    )?;
    ...
    let assumptions = serde_json::to_vec_pretty(&CostAssumptions::empty(manifest.currency.clone()))
    ...
    let report_sections =
        serde_json::to_vec_pretty(&ReportSections::empty()).map_err(|source| {
            WorkspaceError::Json {
                path: report_sections_path.clone(),
                source,
            }
        })?;
...
fn validate_manifest(path: &Path, manifest: &ProjectManifest) -> Result<(), WorkspaceError> {
    ...
    validate_objective(&manifest.objective)
        .map_err(|error| invalid_manifest(path, error.to_string()))?;
