...
) -> Result<Vec<EconomicsScenario>, DomainError> {
    [
        (ScenarioName::Low, assumptions.scenario_prices.low),
        (ScenarioName::Base, assumptions.scenario_prices.base),
        (ScenarioName::High, assumptions.scenario_prices.high),
    ]
    .into_iter()
    .map(|(scenario, price)| {
        calculate_scenario(
            scenario,
            price.ok_or(DomainError::MissingScenarioPrice)?,
            assumptions,
        )
    })
    .collect()
...
fn calculate_scenario(
    ...
    assumptions: &CostAssumptions,
) -> Result<EconomicsScenario, DomainError> {
    let selling = selling_price.decimal();
    if selling <= Decimal::ZERO {
        return Err(DomainError::SellingPriceMustBePositive);
    }
    ...
    let total_cost = assumptions.acquisition_cost.decimal()
        + assumptions.shipping_cost.decimal()
        + marketplace_fee
        + payment_fee
        + assumptions.other_costs.decimal();
    let gross_profit = selling - total_cost;
    let gross_margin_percent = gross_profit / selling * Decimal::from(100);
    Ok(EconomicsScenario {
        scenario,
        selling_price,
        acquisition_cost: assumptions.acquisition_cost,
        shipping_cost: assumptions.shipping_cost,
        marketplace_fee_rate: assumptions.marketplace_fee_rate,
        marketplace_fee: rounded(marketplace_fee),
        payment_fee_rate: assumptions.payment_fee_rate,
        payment_fee: rounded(payment_fee),
        other_costs: assumptions.other_costs,
        total_cost: rounded(total_cost),
        gross_profit: rounded(gross_profit),
        gross_margin_percent: rounded(gross_margin_percent),
    })
}
