...
fn competitor(id: &str, price: Option<&str>) -> Competitor {
    Competitor {
        ...
        product: id.into(),
        brand: String::new(),
        price: price.map(|value| DecimalString::parse(value).unwrap()),
        currency: "INR".into(),
...
fn calculates_even_median_and_average_with_decimal_rounding() {
    let competitors = vec![
        competitor("C-001", Some("100.00")),
        competitor("C-002", Some("200.00")),
        competitor("C-003", Some("400.00")),
        competitor("C-004", Some("900.00")),
    ];
